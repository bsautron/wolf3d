...
/*   Updated: 2015/03/31 06:51:31 by bsautron         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void ft_pt(t_env *env, POS, int y_p, int the_x)
{
	...
	env->img.char_img[x * 4 + (int)y * P0L] =
		env->pic[0].char_img[(int)the_x * 4 + (int)y_p * P_0LINE] + 0.1 * (unsigned char)nuance;
	env->img.char_img[x * 4 + (int)y * P0L + 1] =
		env->pic[0].char_img[(int)the_x * 4 + (int)y_p * P_0LINE + 1] + 0.1 * (unsigned char)(nuance >> 8);
	env->img.char_img[x * 4 + (int)y * P0L + 2] =
		env->pic[0].char_img[(int)the_x * 4 + (int)y_p * P_0LINE + 2] + 0.1 * (unsigned char)(nuance >> 16);
}
